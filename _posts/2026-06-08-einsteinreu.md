...
Notes to self when I write this: mention final conversation with Evra; it doesn't matter that AI solves problems. It gives us time to think about bigger problems. Also, AI could "solve" everything but it doesn't matter because there are always interesting problems that need to be explained well and have intuitive theory. Mathematics is in a way an infinite vacuum, but so far pure mathematics is far ahead of applications, though it still maintains significant purpose. AI could figure everything out, but unless the mathematical community (assuming it values LEAN and human verification equally) regulates and prioritizes communication, there is no actual reason to rush mathematical discovery. AI merely expands the field by making revelations more rapid—we shouldn't treat any given problem as we have for a long time, as being an exercise that "should be doable," as this is the focus of most PhDs. It should be a bigger goal of attempting to extend the reaches of mathematics, and at this point LLMs (aside from AGI) cannot do this. For this reason having an excellent foundation is still crucial as it allows you to specialize when needed and have new intuitional insight when demanded. 